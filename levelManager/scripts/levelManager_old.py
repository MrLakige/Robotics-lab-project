...
def spawn_model(model, pos, name="", ref_frame='world', color=None):
	...
	if isinstance(pos, tuple):
		args = '-urdf -param '+param_name+' -model '+name+' -x '+  str(pos[0])+ ' -y '+  str(pos[1])+ ' -z '+  str(pos[2])
	else:
		args = '-urdf -param '+param_name+' -model '+name+' -x '+  str(pos.position.x)+ ' -y '+  str(pos.position.y)+ ' -z '+  str(pos.position.z)
...
def spawnaLego(brickType=None, rotated=False, position=None):
	...
	print(position)
	spawn_model(brickType, position, name, spawn_name, color)
	lego.append((name, brickType, position, radius))
...
def setUpArea(livello=None, selectBrick=None): 	
	...
	try:
		if(livello == 1):
			...
			spawnaLego(brickType=selectBrick, rotated=True, position=(10, 10, 10))
